(function () {
  ...
  document.getElementById("toolVirtual").onclick = () => apiPost(`/api/devices/${encodeURIComponent(toolboxSerial)}/virtual-display`, {always_on_top:localStorage.getItem("previewAlwaysOnTop")==="1"});
  ...
  function renderDebug(){document.getElementById("toolboxBody").innerHTML=`<div class="package-list"><div class="package-item"><div class="package-name"><b>可视化元素检查器</b><br><span class="muted">截图叠加 UIAutomator 元素边界，可筛选并点击控件</span></div><button class="btn primary" id="openInspector">打开检查器</button></div><div class="package-item"><div class="package-name"><b>页面元素日志</b><br><span class="muted">按设备和时间保存 UTF-8 元素日志</span></div><button class="btn" id="debugDump">立即打印</button></div><div class="package-item"><div class="package-name"><b>独立虚拟屏幕</b><br><span class="muted">通过 scrcpy --new-display 创建不占用手机主屏幕的独立显示</span></div><button class="btn" id="debugVirtual">创建虚拟屏幕</button></div></div>`;document.getElementById("openInspector").onclick=()=>openInspector(toolboxSerial);document.getElementById("debugDump").onclick=()=>apiPost(`/api/devices/${encodeURIComponent(toolboxSerial)}/dump-ui`,{});document.getElementById("debugVirtual").onclick=()=>apiPost(`/api/devices/${encodeURIComponent(toolboxSerial)}/virtual-display`,{always_on_top:localStorage.getItem("previewAlwaysOnTop")==="1"});}
